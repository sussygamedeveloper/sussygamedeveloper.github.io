...
uniform mediump vec3 rgbCoeff;

uniform highp vec2 centerpos;
uniform highp vec2 radius;

void main() {
    mediump vec4 color = texture2D(texture, texCoordinate) * vec4(rgbCoeff, inkParam);

    if(inkEffect == 2) {
        //INVERT
        color.rgb = vec3(1,1,1)-color.rgb;

    } else if(inkEffect == 10) {
        //MONO
        mediump float mono = 0.3125*color.r + 0.5625*color.g + 0.125*color.b;
